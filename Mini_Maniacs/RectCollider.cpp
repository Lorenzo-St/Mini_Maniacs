...
void RectCollider::OnCollision(Entity* other) 
{
  if (tileBased && !Static) 
  {
    ...
    thisPos = { static_cast<float>(static_cast<int>(glm::round(thisPos.x)/tileSize) * tileSize), static_cast<float>(static_cast<int>(glm::round(thisPos.y)/tileSize) * tileSize)  };
